...
def decor_csvfile_user(function: Callable):
...
def json_result_logger(func: Callable):
    result = {}
    if os.path.exists('equation_solutions.json'):
        with open('equation_solutions.json', 'r', encoding='UTF-8') as file:
            result = json.load(file)
    else:
        with open('equation_solutions.json', 'w', encoding='UTF-8') as file:
    ...
    def wrapper(*args):
        ...
        return roots
    print("Решение квадратных уравнений записано в файл equation_solutions.json")
    return wrapper


def create_coefficients_csvfile():
    with open('equation_coefficients.csv', 'w', encoding='UTF-8') as file:
