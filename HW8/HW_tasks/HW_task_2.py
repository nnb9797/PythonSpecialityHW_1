...
def dir_inform(direct: Path, name: str):
    json_data = {}
    ...
    with open(name + '.json', 'w') as f_json, \
        ...
        for dir_path, dir_name, file_name in os.walk(direct):
            ...
            for dirs in dir_name:
                ...
                json_data[dir_path].update({dirs: {'size': size, 'file_or_dir': 'hw_directory'}})
                rows.append({'name': dirs, 'path': dir_path, 'size': size, 'file_or_dir': 'hw_directory'})
